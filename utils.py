...
def load_data(batch_size=1, suffix='', data_folder=None, particle_folder='5particle'):

    data_path = f'data/{particle_folder}/{data_folder}'

    loc_train = np.load(
        f'{data_path}/loc_train{suffix}.npy'
    )
    vel_train = np.load(
        f'{data_path}/vel_train{suffix}.npy'
    )
    edges_train = np.load(
        f'{data_path}/edges_train{suffix}.npy'
    )

    loc_valid = np.load(
        f'{data_path}/loc_valid{suffix}.npy'
    )
    vel_valid = np.load(
        f'{data_path}/vel_valid{suffix}.npy'
    )
    edges_valid = np.load(
        f'{data_path}/edges_valid{suffix}.npy'
    )

    loc_test = np.load(
        f'{data_path}/loc_test{suffix}.npy'
    )
    vel_test = np.load(
        f'{data_path}/vel_test{suffix}.npy'
    )
    edges_test = np.load(
        f'{data_path}/edges_test{suffix}.npy'
    )
