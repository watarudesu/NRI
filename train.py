...
parser.add_argument('--lr-decay', type=int, default=200,
                    help='After how epochs to decay LR.')
...
if args.save_folder:
    ...
    print(f'LOG FILE OPENED: {log_file}', flush=True)
...
off_diag = np.ones(
    [args.num_atoms, args.num_atoms]
) - np.eye(args.num_atoms)

rel_rec = np.array(
    encode_onehot(np.where(off_diag)[0]),
    dtype=np.float32
)
...
# ============================================================
# Load pretrained model ONLY if --load-folder is specified
# ============================================================
...
triu_indices = get_triu_offdiag_indices(
    args.num_atoms
)

tril_indices = get_tril_offdiag_indices(
    args.num_atoms
)
...
def train(epoch, best_val_loss):
    ...
    if log is not None:
        ...
        log.flush()
        os.fsync(log.fileno())
        print(f'LOG WRITTEN: epoch {epoch}', flush=True)
    ...
    for batch_idx, (data, relations) in enumerate(test_loader):
        ...
        if args.decoder == 'rnn':
            ...
            data_plot = data[
                :,
                :,
                args.timesteps:args.timesteps + 21,
                :
            ].contiguous()
